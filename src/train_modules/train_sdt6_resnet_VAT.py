...
def train_resnet_4loss_VAT(input_t, target_Var, decoders, dec_opts, device,
    ...
    for step in range(k, input_time_step - k - BATCH_SIZE + 1, BATCH_SIZE): 
        ...
        print('total_Loss: %.10f' % (super_Loss.item() / input_time_step), 'semi-supervised_Loss: %.10f' % (unlabel_lambda * smsup_Loss.item() / input_time_step), 'entropy_Loss: %.10f' % (en_Loss.item() / input_time_step)) # 
